...
if __name__ == '__main__':
    ...
    print "Send global configuration to set all ASICs to 320Mbps mode"
    for nASIC in range(6):
        # 320Mbps
        # ChValue = ((0x4C<<24)&0xFF000000)+((0xCC<<16)&0x00FF0000)+((00<<8)&0x0000FF00)+((nASIC)&0xFF)
        ChValue = ((0x4C<<24)&0xFF000000)+((0x5C<<16)&0x00FF0000)+((00<<8)&0x0000FF00)+((nASIC)&0xFF)
        # 160Mbps
	#ChValue = ((0x0C<<24)&0xFF000000)+((0x5C<<16)&0x00FF0000)+((00<<8)&0x0000FF00)+((nASIC)&0xFF)
    	cfgValue = []
    	cfgValue.append(ChValue); 
        cfgValue=cfgValue+list(cfgTF.globalc['config'])
