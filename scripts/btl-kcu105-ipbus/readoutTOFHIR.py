...
#    LinkID              = 1
    ...
    while nCycle < int(nCycRead) and Timeout < 10000:
        ...
        for link in range(nlinks):
            ...
            print(channelRead)
        if reduce(operator.or_,channelRead) == 1: # buffer full flag analysis
            nCycle = nCycle + 1;
            #print(nCycle) 
            print("cycle: %d") % nCycle
            ...
            if (1 ):
                channelRead         = [0]*neports
                #resetTimeTagger.write(1)
                #hw.dispatch();
                resetTFHRbufAddrCnt.write(1) # common reset TT and TOFHIR data buffers
                hw.dispatch();
                #resetRxAddressCnts[link].write(1); 
                #hw.dispatch();
                #resetRxAddressCnts[link].write(0); 
                #hw.dispatch();
                print(channelRead)
                TimeOut = 0
        TimeOut = TimeOut + 1 
